...
class AnchorLoader(mx.io.DataIter):
    ...
    def parfetch(self, iroidb):
        # get testing data for multigpu
        data, label = get_rpn_double_batch(iroidb, self.cfg)
        data_shape = {k: v.shape for k, v in data.items()}
        del data_shape['im_info']
        _, feat_shape, _ = self.feat_sym.infer_shape(**data_shape)
        feat_shape = [int(i) for i in feat_shape[0]]
        ...
        # hacking 
        feat_shape[0] = feat_shape[0]/2

        # add gt_boxes to data for e2e
        data['gt_boxes'] = label['all_gt_boxes'][0][np.newaxis, :, :]
        data['ref_gt_boxes'] = label['all_gt_boxes'][1][np.newaxis, :, :]

        # assign anchor for label
        all_labels = []
        for gt_boxes in label['all_gt_boxes']:
            cur_label = assign_anchor(feat_shape, gt_boxes, data['im_info'], self.cfg,
                                  self.feat_stride, self.anchor_scales,
                                  self.anchor_ratios, self.allowed_border,
                                  self.normalize_target, self.bbox_mean, self.bbox_std)
            all_labels.append(cur_label)

        # stack label, bbox_target and bbox_weight
        ret_label = {}
        for key in all_labels[0].keys():
            ret_label[key]  = np.vstack([_[key] for _ in all_labels])
        return {'data': data, 'label': ret_label}
